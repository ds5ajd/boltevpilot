...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      ConstAccel = interp(CS.out.vEgo, [3.0 * CV.KPH_TO_MS, 5.0 * CV.KPH_TO_MS, 8.0 * CV.KPH_TO_MS, 18.0 * CV.KPH_TO_MS, 100.0 * CV.KPH_TO_MS], [0.05, 0.08, 0.11, 0.16, 0.2125])  #0.15, 0.2125
