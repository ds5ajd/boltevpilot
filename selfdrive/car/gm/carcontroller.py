...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      ConstAccel = interp(CS.out.vEgo, [0.1 * CV.KPH_TO_MS, 1.0 * CV.KPH_TO_MS, 3.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 18.0 * CV.KPH_TO_MS, 100.0 * CV.KPH_TO_MS], [0.018, 0.05, 0.06, 0.08, 0.11, 0.15, 0.2125])  #0.15, 0.2125
