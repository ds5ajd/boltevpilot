...
class CarController():
  ...
  def update(self,c,  enabled, CS, controls ,  actuators,
    ...
    if not enabled or not CS.adaptive_Cruise or not CS.CP.enableGasInterceptor:
      ...
      ConstAccel = interp(CS.out.vEgo, [0.5 * CV.KPH_TO_MS, 1.0 * CV.KPH_TO_MS, 3.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 18.0 * CV.KPH_TO_MS, 100.0 * CV.KPH_TO_MS], [0.023, 0.04, 0.05, 0.07, 0.11, 0.15, 0.2125])  #0.15, 0.2125
        ...
        if self.stoppingStateTimeWindowsActive :
          if not self.stoppingStateTimeWindowsClosing:
            ...
            if self.stoppingStateTimeWindowsActiveCounter > 0 :
              actuators.pedalStartingAdder = interp(CS.out.vEgo, [0.0, 2.0 * CV.KPH_TO_MS, 6.0 * CV.KPH_TO_MS, 9.0 * CV.KPH_TO_MS, 12.5 * CV.KPH_TO_MS, 25.0 * CV.KPH_TO_MS, 35.0 * CV.KPH_TO_MS], [0.0, 0.0080, 0.1000, 0.1100, 0.1250, 0.1550, 0.0250]) #0.0050, 0.0200
      ...
      #braking logic
      if actuators.accel < interp(CS.out.vEgo,[2.0* CV.KPH_TO_MS, 6.0* CV.KPH_TO_MS, 10.0* CV.KPH_TO_MS, 18.0* CV.KPH_TO_MS, 30.0* CV.KPH_TO_MS, 60.0* CV.KPH_TO_MS, 80.0* CV.KPH_TO_MS, 100.0* CV.KPH_TO_MS],[-2.2, -2.0, -1.75, -1.5, -1.5, -1.5, -1.5, -1.5]) : #-0.16, -0.6
