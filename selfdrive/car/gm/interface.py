...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    accel_max_bp = [10., 20., 35., 50., 70., 80.]
    accel_max_v = [0.85, 0.92, 0.8, 0.82, 0.92, 0.95] #[0.8, 1.0, 0.95]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=None, disable_radar=False):
    ...
    # longitudinal
    ret.longitudinalTuning.kpBP = [0., 18.0 * CV.KPH_TO_MS, 40.0 * CV.KPH_TO_MS, 80.0 * CV.KPH_TO_MS]
    ret.longitudinalTuning.kpV = [1.16, 1.17, 1.0, 0.76] #[1.10, 1.0, 0.76]
    ret.longitudinalTuning.kiBP = [0., 18.0 * CV.KPH_TO_MS, 40.0 * CV.KPH_TO_MS, 80.0 * CV.KPH_TO_MS]
    ret.longitudinalTuning.kiV = [0.135, 0.130, 0.127, 0.115] #[0.145, 0.125, 0.115]
    
    ret.longitudinalTuning.deadzoneBP = [0., 30.*CV.KPH_TO_MS]
    ret.longitudinalTuning.deadzoneV = [0., 0.10]
    ret.longitudinalActuatorDelayLowerBound = 0.13 #0.13
