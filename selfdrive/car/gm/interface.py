...
class CarInterface(CarInterfaceBase):
  ...
  def get_pid_accel_limits(CP, current_speed, cruise_speed):
    ...
    accel_max_v = [0.85, 0.95, 0.85, 0.82, 1.0, 0.95] #[0.8, 1.0, 0.95]
  ...
  def get_params(candidate, fingerprint=gen_empty_fingerprint(), car_fw=None, disable_radar=False):
    ...
    # longitudinal
    ret.longitudinalTuning.kpBP = [0., 18.0 * CV.KPH_TO_MS, 30.0 * CV.KPH_TO_MS, 80.0 * CV.KPH_TO_MS]
    ret.longitudinalTuning.kpV = [1.16, 1.16, 1.0, 0.76] #[1.10, 1.0, 0.76]
    ret.longitudinalTuning.kiBP = [0., 18.0 * CV.KPH_TO_MS, 30.0 * CV.KPH_TO_MS, 80.0 * CV.KPH_TO_MS]
    ret.longitudinalTuning.kiV = [0.135, 0.137, 0.127, 0.115] #[0.145, 0.125, 0.115]
